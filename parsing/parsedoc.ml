...
let extension_constructor _first file before after ext =
  let docs, after = just_after_comment after in
  let attrs = docs @ parse_attrs ext.pext_attributes in
  let ext = { ext with pext_attributes = attrs } in
    ext, before, after
...
let rec signature_item file before after item =
  ...
  | Psig_typext te ->
      let docs, before = just_before_comment before in
      let item_loc ext = ext.pext_loc in
      let constructors, _, after =
        parse_open_list ~file ~item_loc
           ~parse_item:extension_constructor
           ~before:[] ~after te.ptyext_constructors
      in
      let after_docs, after = just_after_comment after in
      let attrs = docs @ after_docs @ parse_attrs te.ptyext_attributes in
      let te = { te with ptyext_constructors = constructors;
                         ptyext_attributes = attrs }
      in
        { item with psig_desc = Psig_typext te }, before, after
  | Psig_exception ext ->
      let docs, before = just_before_comment before in
      let after_docs, after = just_after_comment after in
      let attrs = docs @ after_docs @ parse_attrs ext.pext_attributes in
      let ext = { ext with pext_attributes = attrs }
      in
        { item with psig_desc = Psig_exception ext }, before, after
  | Psig_module md ->
      let md, before, after =
  ...
  | Psig_class_type cltyds ->
      ...
      let cltyds, before, after =
        parse_open_list ~file ~item_loc
          ...
          ~before ~after cltyds
      in
        { item with psig_desc = Psig_class_type cltyds }, before, after
  | Psig_attribute attr ->
      let attr = parse_attribute attr in
  ...
  | Pstr_typext te ->
      ...
      let exts, _, after =
        parse_open_list ~file ~item_loc
           ...
           ~before:[] ~after te.ptyext_constructors
      in
      let attrs = docs @ parse_attrs te.ptyext_attributes in
      let te = { te with ptyext_constructors = exts;
                         ptyext_attributes = attrs }
      in
        { item with pstr_desc = Pstr_typext te }, before, after
  | Pstr_exception ext ->
      let docs, before = just_before_comment before in
      let attrs = docs @ parse_attrs ext.pext_attributes in
      let ext = { ext with pext_attributes = attrs }
      in
        { item with pstr_desc = Pstr_exception ext }, before, after
  | Pstr_module mb ->
      let mb, before, after =
