...
let rec type_approx env sexp =
  ...
  | Pexp_fun (p, _, _, e) when is_optional p ->
       newty (Tarrow(p, type_option (newvar ()), Placeholder, type_approx env e, Cok))
  | Pexp_fun (p,_,_, e) ->
       newty (Tarrow(p, newvar (), Placeholder, type_approx env e, Cok))
  | Pexp_function ({pc_rhs=e}::_) ->
       newty (Tarrow("", newvar (), Placeholder, type_approx env e, Cok))
...
and type_argument env sarg ty_expected' ty_expected =
  ...
  in
  match expand_head env ty_expected' with
    {desc = Tarrow("",ty_arg,Placeholder,ty_res,_); level = lv} when is_inferred sarg ->
      ...
      let rec make_args args ty_fun =
        ...
        | Tarrow (l,_,Placeholder,ty_res',_) when l = "" || !Clflags.classic ->
