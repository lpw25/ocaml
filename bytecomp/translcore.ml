...
let transl_prim loc prim args =
  ...
  try
    ...
    begin match (p, args) with
        (Psetfield(n, _), [arg1; arg2]) -> Psetfield(n, maybe_addr arg2)
      | (Parraysetu Pmaybe_addr, arg1 :: _) -> Parraysetu(maybe_addr_array arg1)
      | (Parraysets Pmaybe_addr, arg1 :: _) -> Parraysets(maybe_addr_array arg1)
...
and transl_exp0 e =
  ...
  | Texp_setfield(arg, _, lbl, newval) ->
      let access =
        match lbl.lbl_repres with
          Record_regular -> Psetfield(lbl.lbl_pos, maybe_addr newval)
  ...
  then begin
    ...
    let lam =
      ...
      with Not_constant ->
        ...
        | Record_float -> Lprim(Pmakefloatarray, ll) in
    begin match opt_init_expr with
      None -> lam
    ...
    let update_field (_, lbl, expr) cont =
      let upd =
        match lbl.lbl_repres with
          Record_regular -> Psetfield(lbl.lbl_pos, maybe_addr expr)
        | Record_float -> Psetfloatfield lbl.lbl_pos in
