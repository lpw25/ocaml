...
let inline env r ~lhs_of_application
  ...
  let try_inlining =
    if only_use_of_function || always_inline then
      ...
      Dont_try_it S.Not_inlined.Annotation
    else if !Clflags.classic_inlining then
      Dont_try_it S.Not_inlined.Classic_mode
    else if self_call then
      Dont_try_it S.Not_inlined.Self_call
      ...
      in
      Dont_try_it (S.Not_inlined.Function_obviously_too_large threshold)
...
let specialise env r ~lhs_of_application
  ...
  let try_specialising =
    ...
    if !Clflags.classic_inlining then
      ...
      Dont_try_it S.Not_specialised.Annotation
    else if self_call then
      Dont_try_it S.Not_specialised.Self_call
    else if remaining_inlining_threshold = T.Never_inline then
      let threshold =
        match inlining_threshold with
        | T.Never_inline -> assert false
        | T.Can_inline_if_no_larger_than threshold -> threshold
      in
      Dont_try_it (S.Not_specialised.Function_obviously_too_large threshold)
